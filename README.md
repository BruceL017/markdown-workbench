...
> [!NOTE]
> 项目目前处于早期开发阶段。应用功能已在开发分支实现，公开 `main` 暂时只包含项目文档；应用源码整理并合入后，下方的本地运行命令即可直接使用。

## 它解决什么问题

常见的在线 Markdown 工具需要先上传内容，传统编辑器又容易变成过于庞大的开发环境。Markdown Show 面向写作者、开发者、研究者和知识工作者，提供一张安静的浏览器工作台：打开本地文件，按需要分栏，修改后写回原文件或下载副本。
