...
> [!NOTE]
> The project is in early development. The application is implemented on a development branch, while the public `main` branch currently contains project documentation only. The local setup commands below will become directly usable after the application source is prepared and merged.

## What It Solves

Many online Markdown tools require an upload, while traditional editors can become unnecessarily heavy for focused document work. Markdown Show gives writers, developers, researchers, and knowledge workers a quiet browser workbench: open local files, arrange the views you need, then write changes back or download a copy.
...
After the application source is merged into the public branch, the requirements are:
...
The terminal prints the local URL. Build production assets into `dist/` with:
